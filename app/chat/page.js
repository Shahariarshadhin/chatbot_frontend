...
const ChatPage = () => {
  ...
  const { isConnected, messages, joinChat, sendMessage, sendTyping } =
    useSocket();
  ...
  const handleJoin = () => {
    const name =
      userName.trim() || "User_" + Math.random().toString(36).substr(2, 5);
    let userId = localStorage.getItem("chat-userId");
    if (!userId) {
      userId = generateUserId();
      localStorage.setItem("chat-userId", userId);
      ...
      {isWidgetOpen && (
        <div
          className={`chat-widget ${
            isWidgetOpen ? "chat-widget-open" : "chat-widget-minimized"
          }`}
        >
          {!isLoggedIn ? (
            <>
              <div className="chat-header">
                <h2>💬 Live Chat Support</h2>
                <button className="close-widget-btn" onClick={toggleWidget}>
                  ✕
                </button>
              </div>
              <div className="login-form">
